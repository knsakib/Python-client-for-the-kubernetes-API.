...
def kube_api():
    ...
    for i in ret.items:
        ...
        if i.metadata.name not in pod_ip_list:
        # pod_ip_list[j].append(i.status.pod_ip)

            pod_ip_list.append(i.status.pod_ip)
            pod_ip_list.append(i.metadata.namespace)
            pod_ip_list.append(i.metadata.name)
            pod_ip_list.append('new row')
